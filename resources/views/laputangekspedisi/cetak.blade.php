...
<body>
    ...
    <div class="judullaporan">
        <div class="nama-laporan">BUKU UTANG EKSPEDISI</div>
    </div>
        ...
        <table border="1" cellpadding="2" width="100%">
            <thead>
                <tr style="font-size:9px; font-weight: bold;">
                    ...
                    <th width="10%" style="text-align:center;">TGL UTANG</th>
                    <th width="20%" style="text-align:center;">NAMA EKSPEDISI</th>
                ...
                @else
                    ...
                    @foreach ($rsLaporan as $row)
                        <tr style="font-size:9px;">
                            ...
                            <td width="10%" style="text-align:center;">{{ $row->idhutangekspedisi }}</td>
                            <td width="10%" style="text-align:center;">{{ tgldmy($row->tglhutang)  }}</td>
                            <td width="20%" style="text-align:left;">{{ $row->namaekspedisi }}</td>
                            <td width="25%" style="text-align:left;">{{ $row->keterangan }}</td>
                            <td width="10%" style="text-align:right;">{{ format_rupiah($row->kredit) }}</td>
                            <td width="10%" style="text-align:right;">{{ format_rupiah($row->debet) }}</td>
                            <td width="10%" style="text-align:right;">{{ format_rupiah($row->kredit - $row->debet) }}</td>
                        </tr>

                        @php
                            $totalDibayar += $row->debet;
                            $totalHutang += $row->kredit;
